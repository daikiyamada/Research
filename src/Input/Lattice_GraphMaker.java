    ...
    public Graph<MyNode,MyEdge> LatticeGraph_Maker(Graph<MyNode,MyEdge> graph){
        ...
        for(int row = 1; row < latticeSize; row++) {
            ...
            for(int i = 0; i < latticeSize-1; i++) {
                ...
                graph.addEdge(new MyEdge(num_edge,capacity1,cost1), vertexes.get(baseVertex+i), vertexes.get(baseVertex+i-latticeSize)); // 上の頂点と接続
                num_edge++;
                graph.addEdge(new MyEdge(num_edge,capacity2,cost2), vertexes.get(baseVertex+i), vertexes.get(baseVertex+i+1)); // 右の頂点と接続
                num_edge++;
            }
            // 上の頂点と接続. 一番右の頂点は右に頂点は無いので上だけ.
            int capacity = rnd.nextInt(par.link_resource_max-par.link_resource_min)+par.link_resource_min;
            int cost = rnd.nextInt(par.link_cost_max-par.link_cost_min)+par.link_cost_min;
            graph.addEdge(new MyEdge(num_edge,capacity,cost), vertexes.get(baseVertex+(latticeSize-1)), vertexes.get(baseVertex+-1));
            num_edge++;
        }
        return graph;
    }
